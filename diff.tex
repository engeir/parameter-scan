...
%DIF ADD ./agujournaltemplate.tex                             Sat Aug 31 19:05:18 2024
...
%DIFDELCMD <     %%%
\DIFdel{The RF }\DIFdelend \DIFaddbegin \DIFadd{ERF }\DIFaddend to AOD ratio has a time-after-eruption dependence on eruption latitude\DIFaddbegin \DIFadd{.
    }\DIFaddend \item
    ...
    \DIFadd{Temperature and ERF }\DIFaddend peak values has a linear dependence \DIFdelbegin \DIFdel{and reaches an upper limit
  }\DIFdelend \DIFaddbegin \DIFadd{with maximum temperature
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{we do not focus on the M20 data in the GMST plots but
...
\DIFdelend %DIF >    \begin{tikzpicture}[>={Stealth[length=1mm,width=2mm]}]
  ...
  the forcing due to a doubling of pre-industrial \ce{CO2} concentration). \DIFdelbegin %DIFDELCMD < \annote[reviewer1]{As the forcing of volcanic
%DIFDELCMD <     eruptions typically lasts for about a year}{Volcanic forcing definitely lasts for
%DIFDELCMD <     more than a year!}%%%
\DIFdel{, a duration too short }\DIFdelend \DIFaddbegin \DIFadd{As the
  forcing of volcanic eruptions last for too short time }\DIFaddend for \(F=\rho T\) to remain
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The peak value of the J05 ERF is
  ...
  \(e\)-folding time \mbox{%DIFAUXCMD
\cite{clyne2021}}\hskip0pt%DIFAUXCMD
. This result in a sharper peak in AOD and ERF,
...
, with each table entry also indicating the specific model
  ...
  \begin{acronyms}
    ...
    the model'' \acro{FSNT} ``net solar flux at the top of the model'' \DIFaddbegin \acro{GMST}
    \DIFadd{global mean surface temperature }\DIFaddend \acro{IRF} instantaneous radiative forcing
    \acro{MAM3} three mode version of the Modal Aerosol Module \acro{MA} middle
    atmosphere \acro{POP2} Parallel Ocean Program Version 2 \DIFdelbegin %DIFDELCMD < \acro{RF} %%%
\DIFdelend \DIFaddbegin \acro{ERF} \DIFaddend effective
