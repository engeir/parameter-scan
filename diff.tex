...
%DIF ADD ./agujournaltemplate.tex                             Mon Aug 26 18:07:42 2024
...
\DIFdelend \DIFaddbegin \note[reviewer1]{L15}
    ...
    }\DIFaddend anomalies. Unlike the traditional linear \DIFdelbegin \DIFdel{models }\DIFdelend \DIFaddbegin \DIFadd{model }\DIFaddend used for smaller eruptions, \DIFdelbegin \DIFdel{our results reveal }\DIFdelend a
    non-linear relationship \DIFdelbegin \DIFdel{between RF }\DIFdelend \DIFaddbegin \DIFadd{is typically found between ERF }\DIFaddend and AOD for larger eruptions.
    We \DIFdelbegin \DIFdel{also }\DIFdelend \DIFaddbegin \DIFadd{further }\DIFaddend uncover a notable time-dependent decrease in aerosol forcing efficiency
    \DIFaddbegin \DIFadd{(ERF normalised by AOD) }\DIFaddend across all eruption magnitudes during the first
    post-eruption year. In addition, the study reveals that \DIFdelbegin \DIFdel{larger }\DIFdelend \DIFaddbegin \DIFadd{the largest }\DIFaddend as compared to
    \DIFdelbegin \DIFdel{medium-sized }\DIFdelend \DIFaddbegin \DIFadd{Pinatubo-sized }\DIFaddend eruption events produce a delayed and sharper peak in AOD, and a
    longer-lasting \DIFdelbegin \DIFdel{temperature }\DIFdelend \DIFaddbegin \DIFadd{GMST }\DIFaddend response while the time evolution of \DIFdelbegin \DIFdel{RF }\DIFdelend \DIFaddbegin \DIFadd{ERF }\DIFaddend remains similar between
    the two eruption types. \DIFaddbegin \DIFadd{We find that the peak ERF approaches a limiting value, and
    that the peak GMST response follows linearly, effectively bounding the GMST to at
    most \(\sim\SI{-12}{\kelvin}\). }\DIFaddend When including the results of previous studies, we
    find that relating \ce{SO2} to any other parameter is inconsistent across models
    compared to the relationships between AOD, \DIFdelbegin \DIFdel{RF, and temperature
    anomaly}\DIFdelend \DIFaddbegin \DIFadd{ERF, and GMST}\DIFaddend . Thus, we expect the
    largest uncertainty in model codes to relate to \DIFdelbegin %DIFDELCMD < \annote[reviewer1]{the chemistry and physics of \protect\ce{SO2} evolution.}{The
...
\DIFdel{Finally, we find that the peak RF approaches a limiting value,
    ...
    temperature anomaly to at most \(\sim\SI{-12}{\kelvin}\)}\DIFdelend \DIFaddbegin \note[reviewer1]{L25} \DIFadd{the chemistry
    and physics of aerosol evolution}\DIFaddend .
  \end{abstract}
