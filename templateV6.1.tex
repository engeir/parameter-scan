...
in the \gls{cesm2} climate model using the \gls{waccm} atmosphere model. Our analysis
...
for smaller eruptions, our results reveal a non-linear relationship between \gls{rf}
and \gls{aod} for larger eruptions. We also uncover a notable time-dependent decrease in
aerosol forcing efficiency across all eruption magnitudes during the first post-eruption
year. In addition, the study reveals that larger eruption events produce a delayed
and sharper peak in \gls{aod}, and a longer-lasting temperature response while the
time evolution of \gls{rf} remains consistent. When including the results of previous
...
sensitivity \citep{boer2007} by assuming that volcanic and \ce{CO2} forcings produce
...
\ce{SO2} injection magnitudes across four seasons: February 15th, May 15th, August 15th,
...
show values of \ce{SO2}, thus a halfing of the \ce{SO2} values on the axis gives the
...
pre-industrial \ce{CO2} concentration). As forcing of volcanic eruptions typically last
...
in ratio has been attributed to that larger eruptions, injecting more \ce{SO2}, lead to
...
These factors, such as \ce{OH} scarcity and aerosol growth, influencing reflectance and
...
\ce{SO2} \citep{niemeier2015}, but has a wide spread at high values of \iso{} across
...
are too scarce which limit \ce{SO2} oxidation. The \gls{aod} peak in the \gls{ytt}
...
\ce{H2SO4} is represented is an important step in improving the accuracy of volcanic
...
would also be useful as a test to check if a comparison between \ce{SO2} injection
...
eruptions with a given \ce{SO2} amount that is injected during six
...
the peak values of the \ce{SO2} dataset was used in the analysis. Output variables are
available at
