...
\gls{waccm} atmosphere model. Our analysis centres on the impact of injections
...
eruptions. We also uncover a notable time-dependent decrease in aerosol
forcing efficiency across all eruption magnitudes during the first year post-eruption. In addition, the study reveals that
larger volcanic events produce a delayed and sharper peak in \gls{aod}, and a longer-lasting temperature response while the time evolution of \gls{rf} remains consistent. When comparing with previous studies, we find that relating \ce{SO2} to any other parameter is inconsistent across models compared to the relationships between \gls{aod}, \gls{rf} and temperature anomaly, and as such expect the largest uncertainty in model codes to relate to the chemistry and physics of \ce{SO2} evolution.
}
...
Comparison of forcing from volcanoes and \ce{CO2} aims to mitigate the large uncertainty
...
sensitivity \citep{boer2007}, assuming that volcanic and \ce{CO2} forcings produce
...
forcing and \ce{CO2} doubling seem to exist \citep{douglass2006}, or that constraining
...
A notable feature across the subfigures of Fig.~\ref{fig:compare-waveform-temp} is
the peak occurrence of the \gls{c2wm} case compared to the larger eruption
cases. The peak of \gls{c2wm} arrive earlier for both \gls{aod}
(Fig.~\ref{fig:compare-waveform-temp}a) and temperature
(Fig.~\ref{fig:compare-waveform-temp}c), while the \gls{rf} time series in
Fig.~\ref{fig:compare-waveform-temp}b are all indistinguishable. Cases \gls{c2wmp} and
...
same development is not found in the \gls{aod} time series. \gls{c2wm} peak at an
earlier time, but also spend more time around the peak and as such decay later
post-eruption. Similarly, \gls{c2wmp} has a faster rise and slower decay compared to
\gls{c2ws}, but where both peak at a similar time.
...
From Fig.~\ref{fig:compare-waveform-temp} it is clear that the time scale of the
perturbation of \gls{aod} and \gls{rf} is much shorter than that of the temperature.
While the \gls{aod} and \gls{rf} time series are back to their equilibrium state within
roughly three years, the temperature time series are still heavily perturbed three years
post-eruption. Even when allowing the simulations to run for 20 years post-eruption, the
...
% NOTE: can add that the strongest feature in this fig and fig3 is that different
% eruption strength has different ratio. Compared to other eruption strengths, any ratio
% may seem roughly constant, but focusing on a single strength there is a clear aod
% forcing efficiency decrease for tropical eruptions.
...
\begin{figure}
  ...
  \caption{\gls{rf} as a function of \gls{aod}, yearly means. Data from the four
    ...
    eruptions. The peak values from the simulations \gls{c2wm}, \gls{c2wmp} and
    \gls{c2ws} are shown as red circles. Additionally in (a) the simulated super-volcano of \citet{jones2005} (pink square) is
    shown. All peak values (as opposed to annual means) have an asterisk
...
ratio from the first season is not included. Similarly, after three years both time
series are almost fully equilibrated (Fig.~\ref{fig:compare-waveform-temp}a,b). The data
is further divided into two periods; a pre-peak period where the peak of both the \gls{aod}
and the \gls{rf} is included, and a post-peak period for the decaying part.
...
\begin{table}
  ...
  \caption{Slope and standard deviation for the data in
    ...
    Fig.~\ref{fig:aod_vs_toa_avg_loop_ratios}b. The columns ``pre-peak'' and ``post-peak''
    refer to the two periods as shown in Fig.~\ref{fig:aod_vs_toa_avg_loop_ratios}. The
...
\begin{figure}
  ...
  \caption{(a): The ratio of \gls{rf} to \gls{aod}, with time-after-eruption on the
    ...
    the ensembles for each season. Regression fits and shadings are made for the
    pre-peak and post-peak periods. (b): Same as in (a), but where the underlying \gls{aod}
...
%The primary input parameter in the \gls{cesm2} is \iso{}. 
...
%Notably, \gls{ob16} takes a different trajectory compared to \gls{c2w}, showing smaller temperature dependence on \iso{}. 
%In contrast to our findings, \gls{t10} finds a considerably weaker temperature perturbation, noting 
...
\(\mathrm{AOD}<1\) compared to the slopes of approximately
\(\SI{-10}{\watt\metre^{-2}\mathrm{AOD}^{-1}}\) and
...
of magnitude larger injections of \ce{SO2} are found to have an \gls{rf} dependence on
...
%A similar analysis has been carried out before by \citet{marshall2020}, who found that the
...
%well as a similar ratio compared to volcanoes of similar size in our experiments. 
...
consistency of the chemistry and physics of \ce{SO2} and \ce{H2SO4} in models would be
...
% TODO: From abstract. Reformulate
...
% TODO: think about how this might be included
The \gls{waccm} version employed in the \gls{ma} configuration uses the \gls{mam3}
\citep{gettleman2019}, a simplified and computationally efficient default setting within
the \gls{cam5} \citep{liu2016}, as described in \citet{liu2012}. The \gls{mam3} was
...
%Fine sea salt is assimilated into the accumulation mode, and as dust absorbs water efficiently, aerosols are expected to be removed by wet deposition similarly to sea salt, allowing fine dust to be merged into the accumulation mode as well. 
%Likewise, coarse dust is merged with coarse sea salt into a coarse mode, and as both absorb water efficiently, 
