...
larger volcanic events produce a delayed and sharper peak in \gls{aod}, and a longer-lasting temperature resopnse while the ime evolution of $\gls{rf}$ remains consistent.  These findings
...
decay. The rise in RF across the three eruption cases in Fig.~\ref{fig:compare-waveform-temp}b
...
against \iso{}, Fig. 4a. The latitude also plays a role for the magnitude of the \gls{aod}
...
Notably, \gls{ob16} and \gls{t10} have a weaker temperature response \iso{} than \gls{c2w}. 
...
to an inherent non-linear \gls{rf} dependence on \gls{aod}. Both the \gls{g16} data in Fig. 2 and the
...
post-eruption evolution of the ratio. Given that the \gls{c2wsn} case lacka a significant
...
high-latitude case displays an almost constant efficiency with time. These results is in agreement with a reanalysis of the tropical data in \citet{marshall2020dataset}, contrary to the results using all latittudes in the data set \citet{marshall2020}.
...
Considering \iso{} and the peak values of {aod}, \gls{rf} and \gls{}, there is a large spread in the across models, in particular across model families. Improving the consistency of the
...
Allowingfor different latitudes, similar to the \gls{m20} dataset, would also be useful to study
