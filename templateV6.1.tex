...
\title{
Radiative forcing dependency on aerosol optical depth from large and super-volcano
eruptions
}
...
\abstract{%
We investigate the climatic effects of volcanic eruptions spanning from large-scale
events, akin to Mt. Pinatubo, to super-volcanoes, utilising ensemble simulations in the
\gls{cesm2} climate model. Our analysis centres on the impact of varying sulphur dioxide
(\ce{SO2}) injection magnitudes on \gls{aod}, \gls{rf}, and global temperature
anomalies. The results highlight a non-linear relationship between \gls{rf} and
\gls{aod} for larger eruptions, diverging from the traditional linear models used for
smaller eruptions. We also uncover a notable time-dependent decrease in aerosol forcing
efficiency post-eruption, across all eruption sizes. The study reveals that larger
volcanic events produce a delayed and sharper peak in \gls{aod}, alongside a similar
shape in \gls{rf} and temperature time series. These findings emphasise the complexity
of volcanic impacts on climate, demonstrating significant differences in climatic
response depending on eruption magnitude.
}
