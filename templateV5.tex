...
There are three different forcing magnitudes that has been used. The figures below show
tow alternative ways of normalising the temperature response to these volcanic events.

First, we have the three temperature responses normalised by setting the peak value
equal to one, shown in Figure~\ref{fig:temp_norm_max}. The second method of
normalising, shown in Figure~\ref{fig:temp_norm_int}, is to numerically integrate the
...
\begin{figure}
  ...
  \end{center}
  \caption{Normalised temperature response to three different-size volcanic eruptions,
    by means of setting a maximum peak value}
...
\begin{figure}
  ...
  \end{center}
  \caption{
    AOD versus TOA, scaled so the peak values of the different volcano magnitudes
