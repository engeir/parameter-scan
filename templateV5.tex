...
%   the Experimental section (at the level of paragraph subhead- ings) and indicate what
%   information will go.
...
but do tend to have a spread in the \(\mathrm{RF}=\mathrm{AOD}+C\) direction.
...
equal to one, shown in \cref{fig:compare-waveform-max}. The second method of
normalizing, shown in \cref{fig:compare-waveform-integrate}, is to divide by the
integral of the time series, such that the scaled time series integrate to one.
...
black and blue shading in \cref{fig:compare-waveform-max}). However, when normalizing by
