...
\cref{sec:method}. We find that for large to super-volcano sized volcanic eruptions
there is both a clear non-linear \gls{rf} to \gls{aod} dependence, but also a
time-after-eruption dependence on both \gls{aod} and \gls{rf}, which is presented in
\cref{sec:results} and discussed in \cref{sec:discussion}. Finally, in
\cref{sec:conclusions}, conclusions are made.
...
\begin{figure}
  ...
  \caption{Temperature response to the three tropical volcanic eruptions cases,
    ...
    to have (\subref{fig:compare-waveform-max}) peak values at unity and
...
\begin{figure}
  ...
  \caption{The \gls{aod} time series (\subref{fig:aod_arrays_normalised}) and \gls{rf}
    time series (\subref{fig:toa_arrays_normalised}) from the simulations summarised in
...
cases in \cref{tab:simulation-overview}; the small eruption case (\gls{c2wm}) as blue
...
\cref{fig:aod_vs_toa_inset} as blue downward facing triangles, has \gls{rf} values as a
...
\begin{figure}
  ...
  \caption{\gls{rf} as a function of \gls{aod}, yearly means. Data from the four
    simulations listed in \cref{tab:simulation-overview} (\gls{c2wm}, \gls{c2wmp},
...
each other over time, as briefly mentioned in relation to \cref{fig:arrays_normalised},
we plot in \cref{fig:aod_vs_toa_avg_loop_ratios} seasonal means of the \gls{rf} to
\gls{aod} ratio, where the start of the time series is taken as that of the eruption
day. \Cref{fig:aod_vs_toa_avg_loop_ratio,fig:aod_vs_toa_avg_loop_ratio_scaled} show the
\gls{rf} to \gls{aod} ratios from \cref{fig:aod_vs_toa_full,fig:aod_vs_toa_inset}, but
...
\cref{fig:toa_arrays_normalised}). Hence, the ratio of \gls{rf} to \gls{aod} is
...
\begin{figure}
  ...
  \caption{(\subref{fig:aod_vs_toa_avg_loop_ratio}): Ratio of \gls{rf} to \gls{aod}, with
    ...
    Same as in \subref{fig:aod_vs_toa_avg_loop_ratio}, but where the underlying \gls{aod}
...
in \cref{fig:so2_vs_aod}. The latitude also plays a role for how large the \gls{aod}
...
\gls{rf}. \iso[I] versus \gls{rf} is shown in \cref{fig:so2_vs_toa}, but where the
absolute value of the \gls{rf} is potted along the \(y\)-axis. Just as in
\cref{fig:aod_vs_toa_full} we find the \gls{cesm2} data points to be heavily damped in
...
\cref{app:ob16}. The simulations used by \citet{ottobliesner2016} was using \gls{cesm1}
...
%     \gls{rf} and (e) temperature as a function of \gls{aod}. (f) Temperature as a function of
%     \gls{rf}. \gls{c2w} cases are shown as blue diamonds, the \gls{c2wsn} case is the brown
...
\begin{figure*}
  ...
  \caption{(\subref{fig:so2_vs_aod}) \gls{aod} (\subref{fig:so2_vs_toa}) \gls{rf} and
    (\subref{fig:so2_vs_temp}) temperature as a function of \iso{}\@.
    (\subref{fig:aod_vs_toa}) \gls{rf} and (\subref{fig:aod_vs_temp}) temperature as a
    function of \gls{aod}. (\subref{fig:toa_vs_temp}) Temperature as a function of \gls{rf}.
    \gls{c2w} cases are shown as blue diamonds, the \gls{c2wsn} case is the brown
    three-branched twig, OB16 are data from \citet{ottobliesner2016} marked as red
    asterisks, the purple star and yellow plus are Mt.\ Pinatubo and Mt.\ Tambora estimates,
    the pink square is the one-hundred times Mt.\ Pinatubo super-volcano from
    \citet{jones2005} and the pink disk is the \gls{ytt} super-volcano from
...
\cref{fig:so2_vs_toa}). This is in good agreement with the functional given in
\cref{eq:niemeier_exponential}.
...
\cref{fig:so2_vs_toa}, the data from the \gls{cesm2} simulations bend off, making the
...
In \cref{fig:aod_vs_toa} we again look at \gls{rf} as a function of \gls{aod}, but this
...
\Cref{fig:aod_vs_temp} should in the case of the \gls{cesm2} data points be akin in
shape to the paths found in \cref{fig:so2_vs_temp} due to the close to linear
relationship found between \iso{} and \gls{aod} in \cref{fig:so2_vs_aod}. The
...
In \cref{fig:toa_vs_temp} we compare the \gls{rf} and temperature responses. Both the
...
\Cref{fig:aod_vs_toa_ses_avg,fig:aod_vs_toa_avg_loop_ratios,fig:aod_vs_toa} all show
that as the \gls{aod} increases past \(\sim 1.0\), the scaling of \(\sim -20\) no longer
map to corresponding values of \gls{rf}. When looking at the intermediate volcanic
...
\gls{rf} values. From \cref{fig:so2_vs_aod} we have an almost linear relation between
\iso{} and \gls{aod}, and previous results indicate that larger eruptions where more
...
very large eruptions, they look at Pinatubo and \gls{ytt}, \ce{OH} radicals are not
...
though the peak is representable, as we have seen in \cref{fig:aod_arrays_normalised}
...
(\cref{fig:so2_vs_aod,fig:so2_vs_toa,fig:so2_vs_temp}). Compared to \gls{aod} as a
function of \iso{}, both \gls{rf} (\cref{fig:aod_vs_toa}) and temperature
(\cref{fig:aod_vs_temp}) as a function of \gls{aod} result in a small spread in the data
across models, and necessarily the spread will also be small for temperature as a
function of \gls{rf} (\cref{fig:toa_vs_temp}). A roughly linear relationship between
...
relationship between \gls{aod} and \gls{rf} emerge in \cref{fig:aod_vs_toa_ses_avg}.
...
\cref{fig:aod_vs_toa_avg_loop_ratios}. \citet{marshall2020} explain that this is due to
