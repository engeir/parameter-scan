...
perturbation. We also note that \citet{timmreck2010} obtain cooling that is much smaller
than for example \citet{jones2005}.
...
of \(\sim -20\) no longer map to corresponding values of \acrshort{rf}. When
looking at the intermediate volcanic eruption with \(\SI{400}{\tera\gram}\) \iso{} there
seem to be a mostly linear relationship between the \acrshort{aod} and \acrshort{rf}
values, with a slope of approximately \(-10\). As for the largest eruption, scaling
\acrshort{aod} by \(\sim-4\) yields reasonable \acrshort{rf} values. From
\cref{fig:so2_vs_aod} we have an almost linear relation between \iso{} and
\acrshort{aod}, and previous results indicate that larger eruptions where more \ce{SO2}
is injected creates larger aerosols which are less effective at scattering radiation,
making the cooling less efficient \citep{english2013,timmreck2010,timmreck2018}.
(\citet{timmreck2010} mention that for very large eruptions, they look at Pinatubo and
Young Toba Tuff, \ce{OH} radicals are not abundant enough which limit the \ce{SO2}
oxidation. Originally in \citep{bekki1995}.)

Only when inspecting the data generated from the largest eruption at
\(\SI{1629}{\tera\gram}\) of \iso{} can we see that it initially follow approximately
the same path as the intermediate, before it continues on a much less steep path.
Nevertheless, the shape the two eruptions draw is very similar when scaled, suggesting
that the intermediate also do follow a loop rather than there just being noise that
causes the spread. That is, there appear to be a time-after-eruption dependency on the
ratio between \acrshort{aod} and \acrshort{rf}. \citet{marshall2020} discuss a similar
feature they find when looking at \acrshort{aod} (\acrshort{saod}) and \acrshort{rf},
but there, the efficiency increases from year 1 to year 2, while here efficiency
decreases with time. \citet{marshall2020} explain that this is due to the time it takes
for the aerosols to spread which affects the global albedo and in turn the
...
and estimates from then give an \acrshort{rf} to \acrshort{aod} that is quite steep,
while larger eruptions (or values of \iso{} in the atmosphere, see \citet{niemeier2015})
result in estimates that are smaller (\(\sim-20\) versus \(\sim-10\) to \(\sim-4\)). It
has been shown that that efficiency decrease as the injection rate increase
\citep{niemeier2017}, related to the \acrshort{qbo}, and that large volcanic eruptions
result in larger aerosol particle sizes which result in decreasing cooling efficiency
per \iso{} \citep{english2013, timmreck2018}. Can this be used to reason that the
efficiency is initially high, then decreasing?

Section about how RF to AOD show non-linear trend when going past \(\sim\) 2 in AOD and
-20 in RF\@. Also that the slope already at this point is -10 rather than the
\(\sim\)-20 used many other places. Why?

Section about the apparent loop that shows up in the RF to AOD plot, and how it also
looks to loop opposite to what Marshall et al 2020 found. Albedo versus other effects,
like aerosol size, growth \& sedimentation phases\ldots{}
